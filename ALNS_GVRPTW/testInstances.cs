...
namespace ALNS_GVRPTW
{
    class TestInstances//运行算例，验证算法
    {
        ...
        public bool CheckSolution(List<Route> solution, List<Request> CustomerSet, List<List<double>> Cij, List<List<double>> Tij)//
        {
            ...
            {
                number += (solution[i].route.Count - 1);
            }
            for (int i = 0; i < solution.Count; i++)
            {
                solution[i].routeCost = 0;
                solution[i].routeCapacity = 0;
                solution[i].visitTime.RemoveRange(1, solution[i].route.Count - 1);
                double td;//行驶时间
                int j;
                for (j = 1; j < solution[i].route.Count; j++)
                {
                    double Ta;
                    solution[i].routeCost += (A1 + A2 * (Car_weight + solution[i].routeCapacity) / 1000) * Cij[solution[i].route[j - 1]][solution[i].route[j]] * MS;//碳排放
                    if (j == 1)
                    {
                        Ta = Tij[0][solution[i].route[j]];//车场没有服务时间
                    }
                    else
                    {
                        Ta = solution[i].visitTime[j - 1] + ServiceTime + Tij[solution[i].route[j - 1]][solution[i].route[j]];
                    }
                    double Tmin = CustomerSet[solution[i].route[j]].earlyTime;
                    double Tmax = CustomerSet[solution[i].route[j]].lastTime;
                    if (Ta <= Tmax)
                    {
                        if (Ta <= Tmin)
                        {
                        ...
                        solution[i].routeCapacity += CustomerSet[solution[i].route[j]].cusneed;
                    }
                    else
                    {
                        suceess = false;
                ...
                if (suceess && solution[i].visitTime[j - 1] + ServiceTime + Tij[solution[i].route[j - 1]][0] > CustomerSet[0].lastTime)
                {
                    suceess = false;
                    Console.WriteLine("第{0}route的{1}节点违法车场时间窗约束！,超出：{2}", i, solution[i].route[j - 1], CustomerSet[0].lastTime - solution[i].visitTime[j - 1] - ServiceTime - Tij[solution[i].route[j - 1]][0]);
                }
                if (solution[i].routeCapacity > Capacity)
                {
                    ...
                    solution[i].routeCost += (A1 + A2 * (Car_weight + solution[i].routeCapacity) / 1000) * Cij[solution[i].route[j - 1]][0] * MS;//碳排放
            }
            double c = 0;
            int L = Length - 1;
            if (number != L)
            {
                ...
                for (int t = 0; t < node.Count; t++)
                {
                    ...
                    for (int i = 0; i < solution.Count; i++)
                    {
                        ...
                        for (int j = 1; j < solution[i].route.Count; j++)
                        {
                            if (solution[i].route[j] == node[t])
                            {
                                a = i;
                                count++;
                            }
